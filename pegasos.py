...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        z_sum += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    return z_sum/feature_matrix.shape[0]
...
def perceptron(feature_matrix, labels, T):
    ...
    Args:
        feature_matrix -  A numpy matrix describing the given data. Each row
            represents a single di],ata point.
        labels - A numpy array where the kth element of the array is the
            correct classification of the kth row of the feature matrix.
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    pred_array = np.zeros(feature_matrix.shape[0])
    for i in range(feature_matrix.shape[0]):
...
def bag_of_words(texts):
    ...
    for text in texts:
        ...
        for word in word_list:
            if (word not in dictionary) and (word not in stoplist): # uncomment the and (word not in stoplist)
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                feature_matrix[i, dictionary[word]] = word_list.count(word) # change this to 1 to be binary
